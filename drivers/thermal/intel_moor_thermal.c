...
static void release_iio_env(void)
{
	int i;

	for (i = 0; i < tdata->num_chans; i++) {
		if (tdata->iio_chans[i])
			iio_channel_release(tdata->iio_chans[i]);

		/* kfree(NULL) is safe */
		kfree(tdata->new_names[i]);
	}

	kfree(tdata->iio_chans);
	kfree(tdata->new_names);
	kfree(tdata->cached_vals);
}

/* setup_iio_env - sets up all the IIO related environment
 * @dev: device pointer
 */
static int setup_iio_env(struct device *dev)
{
	int i, ret = 0;
	int size;
	struct iio_channel *iio_chan;

	dev_dbg(dev, "dev_name used for iio: %s\n", dev_name(dev));
	iio_chan = iio_channel_get_all(dev);
	if (iio_chan == NULL) {
		dev_err(dev, "Unable to get_all for iio_chan\n");
		return -EINVAL;
	}

	/* Check whether we got all the four channels */
	tdata->num_chans = iio_channel_get_num(iio_chan);
	dev_info(dev, "Number of iio channels:%d\n", tdata->num_chans);
	if (tdata->num_chans > 0) {
		tdata->new_names = kzalloc(sizeof(char *) * tdata->num_chans,
				GFP_KERNEL);
		if (tdata->new_names == NULL) {
			dev_err(dev,
				"kzalloc failed for new_names\n");
			ret = -ENOMEM;
			goto exit_rel;
		}

		for (i = 0; i < tdata->num_chans; i++) {
			tdata->new_names[i] = kzalloc(sizeof(char) * 24,
					GFP_KERNEL);
			if (tdata->new_names[i] == NULL) {
				dev_err(dev,
					"kzalloc failed for new_names[%d]\n",
					i);
				ret = -ENOMEM;
				goto exit_err;
			}
	...
	ret = iio_channel_get_name(iio_chan, tdata->new_names);
	if (ret) {
		dev_err(dev, "Error in getting channel-name:%d\n",
				ret);
		goto exit_err;
	}

	tdata->cached_vals = kzalloc(sizeof(int) * tdata->num_chans,
			GFP_KERNEL);
	if (tdata->cached_vals == NULL) {
		dev_err(dev, "kzalloc failed for cached_vals\n");
		ret = -ENOMEM;
		goto exit_err;
	}

	size = sizeof(struct iio_channel *) * tdata->num_chans;
	tdata->iio_chans = kzalloc(size, GFP_KERNEL);
	if (tdata->iio_chans == NULL) {
		dev_err(dev, "kzalloc failed for iio_chans\n");
		ret = -ENOMEM;
		goto exit_err;
	}

	for (i = 0; i < tdata->num_chans; i++) {
		dev_info(dev, "Channel-name[%d]:%s\n", i, tdata->new_names[i]);
		tdata->iio_chans[i] = iio_channel_get(NULL,
				tdata->new_names[i]);
		if (tdata->iio_chans[i] == NULL) {
			dev_err(dev, "tdata->iio_chans null for %s\n",
					tdata->new_names[i]);
			ret = -EINVAL;
			goto exit_err;
		}
	...
	/* After setup done, iio_chan is not needed.
	 * It's only used to get the number and name of channels */
	goto exit_rel;

exit_err:
	dev_err(dev, "IIO environment setup failed with err:%d\n", ret);
	release_iio_env();
exit_rel:
	iio_channel_release_all(iio_chan);
...
static int program_tmax(struct device *dev)
{
	 ...
	 */
	for (i = 0; i < tdata->num_chans; i++) {
		cur_chan = tdata->iio_chans[i];

		dev_dbg(dev, "%s: write_event_ falling:%d for chan:%x\n",
				__func__, DEFAULT_MAX_TEMP,
				cur_chan->channel->channel);
		if (cur_chan->indio_dev->info->write_event_value)
			ret = cur_chan->indio_dev->info->write_event_value(
					cur_chan->indio_dev,
					cur_chan->channel, IIO_EV_TYPE_THRESH,
					IIO_EV_DIR_FALLING, IIO_EV_INFO_VALUE,
					DEFAULT_MAX_TEMP, 0);

		if (ret < 0)
			goto exit_err;

		dev_dbg(dev, "%s: write_event_ rising:%d for chan:%x\n",
				__func__, DEFAULT_MAX_TEMP,
				cur_chan->channel->channel);
		if (cur_chan->indio_dev->info->write_event_value)
			ret = cur_chan->indio_dev->info->write_event_value(
					cur_chan->indio_dev,
					cur_chan->channel, IIO_EV_TYPE_THRESH,
					IIO_EV_DIR_RISING, IIO_EV_INFO_VALUE,
					DEFAULT_MAX_TEMP, 0);
...
exit_err:
	dev_err(dev, "set alert for channel %d failed:%d\n",
			cur_chan->channel->channel, ret);
	return ret;
}
				...
				int trip, long hyst)
{
	...
	struct iio_channel *cur_chan = tdata->iio_chans[indx];
	...
	if (trip == 0) {
		dev_dbg(&tzd->device,
				"%s: write_event_ falling:%ld for chan:%x\n",
				__func__, hyst, cur_chan->channel->channel);
		if (cur_chan->indio_dev->info->write_event_value)
			ret = cur_chan->indio_dev->info->write_event_value(
					cur_chan->indio_dev,
					cur_chan->channel, IIO_EV_TYPE_THRESH,
					IIO_EV_DIR_FALLING, IIO_EV_INFO_HYSTERESIS,
					hyst, 0);
	} else {
		dev_dbg(&tzd->device,
				"%s: write_event_ rising:%ld for chan:%x\n",
				__func__, hyst, cur_chan->channel->channel);
		if (cur_chan->indio_dev->info->write_event_value)
			ret = cur_chan->indio_dev->info->write_event_value(
					cur_chan->indio_dev,
					cur_chan->channel, IIO_EV_TYPE_THRESH,
					IIO_EV_DIR_RISING, IIO_EV_INFO_HYSTERESIS,
					hyst, 0);
	}

	/* Return value is 1 to indicate INT-type */
	if (ret > 0)
		ret = 0;
	...
	if (trip == 0) {
		if (cur_chan->indio_dev->info->read_event_value)
			ret = cur_chan->indio_dev->info->read_event_value(
					...
					cur_chan->channel, IIO_EV_TYPE_THRESH,
					IIO_EV_DIR_FALLING,
					IIO_EV_INFO_HYSTERESIS, &val, &val2);
		dev_dbg(&tzd->device,
				"%s: read_event_ falling:%d for chan:%x\n",
				__func__, val, cur_chan->channel->channel);
	} else {
		if (cur_chan->indio_dev->info->read_event_value)
			ret = cur_chan->indio_dev->info->read_event_value(
					cur_chan->indio_dev,
					...
					IIO_EV_INFO_HYSTERESIS, &val, &val2);
		dev_dbg(&tzd->device,
				"%s: read_event_ rising:%d for chan:%x\n",
				__func__, val, cur_chan->channel->channel);
	}
...
static int update_temp(struct thermal_zone_device *tzd, long *temp)
{
	...
	if (!tdata->iio_chans[indx]) {
		dev_err(&tzd->device, "%s: no iio_chans for indx:%d\n",
				__func__, indx);
		return -EINVAL;
	}
	...
	if (!tdata->is_initialized ||
		...
		for (i = 0; i < tdata->num_chans; i++) {
			ret = iio_read_channel_raw(tdata->iio_chans[i],
					&tdata->cached_vals[i]);
			if (ret) {
				dev_err(&tzd->device,
					"ADC read err:%d for channel[%d]:%s\n",
					ret, i, tdata->new_names[i]);
			}
		}
	...
	*temp = tdata->cached_vals[indx];

	dev_dbg(&tzd->device, "show_temp: TEMP:%ld from ADC:%x for %s\n",
			*temp, tdata->cached_vals[indx], tzd->type);
	return ret;
}
...
static int moor_thermal_probe(struct platform_device *pdev)
{
	...
	ret = setup_iio_env(&pdev->dev);
	if (ret) {
		dev_err(&pdev->dev, "IIO setup failed:%d\n", ret);
		goto exit_tzd;
	}
